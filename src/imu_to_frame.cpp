...
std::string p_stabilized_base_frame_;
...
int main(int argc, char **argv) {
  ...
  pn.param("stabilized_base_frame", p_stabilized_base_frame_, std::string("base_stabilized"));
  pn.param("base_frame", p_base_frame_, std::string("base_link"));
  
  tfB_ = new tf::TransformBroadcaster();
  transform_.getOrigin().setX(0.0);
  transform_.getOrigin().setY(0.0);
  transform_.getOrigin().setZ(0.22);
  transform_.frame_id_ = p_stabilized_base_frame_;
  transform_.child_frame_id_ = p_base_frame_;

  ros::Subscriber imu_subscriber = n.subscribe("imu_topic", 100, imuMsgCallback);
